...
| AT-UI-001 | Pass | Streamlit AppTest loaded all four input modes and all three Manual tables, rendered reactive maps for built-in, generated, manual, and uploaded data, and solved the ten-location Wellington default without exceptions. |
| AT-UI-002 | Pass | All UI ingestion paths use tested whole-file validation and show caught validation errors before solving. |
| AT-UI-003 | Pass | `map_view` is tested with non-Wellington coordinates and derives center/zoom from all coordinates; Streamlit tests verify marker counts update after generator changes and CSV uploads. |
